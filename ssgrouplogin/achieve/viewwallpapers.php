...
if($id != ''){
    try{
        ...
        $sql7 = "DELETE FROM catlink WHERE d_id = '$did'";
        $pdo->exec($sql7);

        $sql8 = "DELETE FROM subcatlink WHERE d_id = '$did'";
        $pdo->exec($sql8);

        $sql9 = "DELETE FROM tagdetails WHERE d_id = '$did'";
        $pdo->exec($sql9);
...
if($tagcount > 0){
    ...
    AND (d.tag LIKE '%$search%' OR td.tagname = '$search')
    ORDER BY d.createdat DESC";
    $result1 = $pdo->query($sql1);
    while($row1 = $result1->fetch()){
        if($row1['width'] == 1280 && $row1['height'] == 720){
...
if($catcount > 0 && $tagcount > 0){
    ...
    AND r.width = '1280' AND r.height = '720'
    AND (d.tag LIKE '%$search%' OR td.tagname = '$search' OR c.cat_name LIKE '%$search%')
    ORDER BY d.createdat DESC";
    $result1 = $pdo->query($sql1);
    while($row1 = $result1->fetch()){
        if($row1['width'] == 1280 && $row1['height'] == 720){
...
if($subcount > 0 && $tagcount > 0){
    ...
    AND r.width = '1280' AND r.height = '720'
    AND (d.tag LIKE '%$search%' OR td.tagname = '$search' OR s.sub_name LIKE '%$search%')
    ORDER BY d.createdat DESC";
    $result1 = $pdo->query($sql1);
    while($row1 = $result1->fetch()){
        if($row1['width'] == 1280 && $row1['height'] == 720){
                            ...
                            <div class = 'col-lg-4' style = 'margin-left: -5px; margin-top: 2%;'>
                                ...
                                <div class = 'row'>
                                    <div class = 'col-md- 3 col-lg-4'>
                                        <a href = 'editwallpaper.php?id=$row[d_id]' class = 'btn btn-primary' id = 'edit'><i class='fa fa-edit'></i></a>
                                    </div>
                                    ...
                                    <div class = 'col-md- 3 col-lg-4'>
                                        <a href = 'wallpaperdetails.php?id=$row[d_id]' class = 'btn btn-info' id = 'details'><i class='fa fa-eye'></i></a>
                                    </div>
                                </div>
                            </div>";
                        }
